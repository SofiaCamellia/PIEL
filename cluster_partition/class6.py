...
def get_data_by_partition():
    ...
    for (min_lon, max_lon, min_lat, max_lat) in regions_to_remove:
        in_region = (pts_lon_conv >= min_lon) & (pts_lon_conv <= max_lon) & \
                    (pts_lat >= min_lat) & (pts_lat <= max_lat)
        remove_mask = remove_mask | in_region

    if np.any(remove_mask):
        print(f"⚠️ Removing {np.sum(remove_mask)} points from excluded regions...")
...
def run_pinn(p_stack_tr, X_tr, y_tr, p_stack_val, X_val, y_val, p_stack_test, X_test, y_test):
    ...
    for epoch in range(50):
        ...
        if v_loss < best_loss: best_loss = v_loss; torch.save(model.state_dict(), f"{BASE_PATH}/models/best_pinn.pth")
...
def main():
    t0 = time.time()
    print("===== PARTITION INTEGRATED PIPELINE STARTED =====")

    # 1. Data (Using New Partition Logic)
    data = get_data_by_partition()
    ...
    p_stack_tr, p_stack_val, p_stack_test, stacker_model = run_stacker(X_sp_tr, Z_tr, y_tr, X_sp_val, Z_val, y_val,
                                                                       X_sp_test, Z_test)
    evaluate(y_test, p_stack_test, "Stacker (Test)")

    p_pinn_test = run_pinn(p_stack_tr, X_tr, y_tr, p_stack_val, X_val, y_val, p_stack_test, X_test, y_test)

    print("\n===== FINAL RESULTS =====")
    evaluate(y_test, p_stack_test, "Stacker Only")
    evaluate(y_test, p_pinn_test, "PINN (Final)")

    np.savez(f"{BASE_PATH}/results/final_predictions.npz", stacker=p_stack_test, pinn=p_pinn_test, y_true=y_test)
