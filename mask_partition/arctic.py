# -*- coding: utf-8 -*-
"""
ARCTIC INTEGRATED PIPELINE:
Geographic Filter -> 6 Base Models -> Spatial Stacker -> PINN
------------------------------------------------------------
1. Filters IAP data for Arctic Ocean using shapefile.
...
REGION = "arctic"  # <--- 在这里定义区域名称，方便以后改为 "north_atlantic" 等
# =======================
# PATHS (请根据实际情况修改)
# =======================
# 原始数据路径
...
# =====================================================================================
# PART 1: Data Filtering (Arctic Region)
# =====================================================================================
def create_arctic_mask(shapefile_path):
    print(">>> Step 1: Creating Arctic geographic mask...")
    try:
        marine_gdf = gpd.read_file(shapefile_path, engine="pyogrio")  # engine="pyogrio" faster
    except Exception:
        marine_gdf = gpd.read_file(shapefile_path)
    ...
    # Flatten mapping
    target_seas = []
    for k, v in ocean_mapping.items(): target_seas.extend(v)

    # Filter
    arctic_gdf = marine_gdf[marine_gdf['name'].isin(target_seas)].copy()
    if arctic_gdf.empty:
        raise ValueError("No Arctic polygons found. Check shapefile or naming.")

    return arctic_gdf.unary_union
...
def get_arctic_data():
    ...
    # 3. Geo Filter
    arctic_mask = create_arctic_mask(SHAPEFILE_PATH)
    print(f"Filtering {len(data)} points by Arctic mask...")
    ...
    mask_gdf = gpd.GeoDataFrame(geometry=[arctic_mask], crs="EPSG:4326")
    ...
    print(f"Arctic Data Count: {len(final_data)}")
...
def main():
    t0 = time.time()
    print("===== ARCTIC INTEGRATED PIPELINE STARTED =====")

    # 1. Data
    data = get_arctic_data()
