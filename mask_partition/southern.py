# -*- coding: utf-8 -*-
"""
southern INTEGRATED PIPELINE:
Geographic Filter -> 6 Base Models -> Spatial Stacker -> PINN
------------------------------------------------------------
1. Filters IAP data for southern Ocean using shapefile.
...
REGION = "southern"  # <--- 在这里定义区域名称，方便以后改为 "north_atlantic" 等
# =======================
# PATHS (请根据实际情况修改)
# =======================
# 原始数据路径
...
# =====================================================================================
# PART 1: Data Filtering (southern Region)
# =====================================================================================
def create_southern_mask(shapefile_path):
    """
    Loads marine polygons and creates a single geometric mask for the southern Ocean region.
    """
    print("Step 1: Creating precise southern geographic mask...")
    try:
    ...
    southern_gdf = marine_gdf[marine_gdf['main_basin'] == 'southern Ocean'].copy()
    if southern_gdf.empty:
        print("No southern polygons found in the shapefile.")
        return None
    southern_mask = southern_gdf.unary_union
    print("southern geographic mask created successfully!")
    return southern_mask


def get_southern_data():
    # 1. Load Raw
    ...
    # 3. Geo Filter
    southern_mask = create_southern_mask(SHAPEFILE_PATH)
    print(f"Filtering {len(data)} points by southern mask...")
    ...
    mask_gdf = gpd.GeoDataFrame(geometry=[southern_mask], crs="EPSG:4326")
    ...
    print(f"southern Data Count: {len(final_data)}")
...
def main():
    t0 = time.time()
    print("===== southern INTEGRATED PIPELINE STARTED =====")

    # 1. Data
    data = get_southern_data()
