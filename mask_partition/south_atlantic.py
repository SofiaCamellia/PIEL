# -*- coding: utf-8 -*-
"""
south_atlantic INTEGRATED PIPELINE:
Geographic Filter -> 6 Base Models -> Spatial Stacker -> PINN
------------------------------------------------------------
1. Filters IAP data for south_atlantic Ocean using shapefile.
...
TARGET_OCEAN_REGION = "South Atlantic Ocean"  # <<<--- 正式名称

# The 'REGION' variable for file paths is now generated automatically.
REGION = TARGET_OCEAN_REGION.lower().replace(" ", "_").replace("_ocean", "") # <<<--- 自动生成简称 "south_atlantic"
# =======================
# PATHS (请根据实际情况修改)
# =======================
# 原始数据路径
...
# =====================================================================================
# PART 1: Data Filtering (south_atlantic Region)
# =====================================================================================
def create_south_atlantic_mask(shapefile_path, target_region):
    """
    Loads marine polygons and creates a single geometric mask for the south_atlantic Ocean region.
    """
    print("Step 1: Creating precise south_atlantic geographic mask...")
    try:
...
def get_south_atlantic_data():
    ...
    # 3. Geo Filter
    # south_atlantic_mask = create_south_atlantic_mask(SHAPEFILE_PATH)
    south_atlantic_mask = create_south_atlantic_mask(SHAPEFILE_PATH, TARGET_OCEAN_REGION)
    print(f"Filtering {len(data)} points by south_atlantic mask...")
    ...
    mask_gdf = gpd.GeoDataFrame(geometry=[south_atlantic_mask], crs="EPSG:4326")
    ...
    print(f"south_atlantic Data Count: {len(final_data)}")
...
def main():
    t0 = time.time()
    print("===== south_atlantic INTEGRATED PIPELINE STARTED =====")

    # 1. Data
    data = get_south_atlantic_data()
