# -*- coding: utf-8 -*-
"""
north_pacific INTEGRATED PIPELINE:
Geographic Filter -> 6 Base Models -> Spatial Stacker -> PINN
------------------------------------------------------------
1. Filters IAP data for north_pacific Ocean using shapefile.
...
TARGET_OCEAN_REGION = "North Pacific Ocean"  # <<<--- 正式名称

# The 'REGION' variable for file paths is now generated automatically.
REGION = TARGET_OCEAN_REGION.lower().replace(" ", "_").replace("_ocean", "") # <<<--- 自动生成简称 "north_pacific"
# =======================
# PATHS (请根据实际情况修改)
# =======================
# 原始数据路径
...
# =====================================================================================
# PART 1: Data Filtering (north_pacific Region)
# =====================================================================================
def create_north_pacific_mask(shapefile_path, target_region):
    """
    Loads marine polygons and creates a single geometric mask for the north_pacific Ocean region.
    """
    print("Step 1: Creating precise north_pacific geographic mask...")
    try:
...
def get_north_pacific_data():
    ...
    # 3. Geo Filter
    # north_pacific_mask = create_north_pacific_mask(SHAPEFILE_PATH)
    north_pacific_mask = create_north_pacific_mask(SHAPEFILE_PATH, TARGET_OCEAN_REGION)
    print(f"Filtering {len(data)} points by north_pacific mask...")
    ...
    mask_gdf = gpd.GeoDataFrame(geometry=[north_pacific_mask], crs="EPSG:4326")
    ...
    print(f"north_pacific Data Count: {len(final_data)}")
...
def main():
    t0 = time.time()
    print("===== north_pacific INTEGRATED PIPELINE STARTED =====")

    # 1. Data
    data = get_north_pacific_data()
