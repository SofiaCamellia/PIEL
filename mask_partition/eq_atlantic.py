# -*- coding: utf-8 -*-
"""
equatorial_atlantic INTEGRATED PIPELINE:
Geographic Filter -> 6 Base Models -> Spatial Stacker -> PINN
------------------------------------------------------------
1. Filters IAP data for equatorial_atlantic Ocean using shapefile.
...
TARGET_OCEAN_REGION = "Equatorial Atlantic Ocean"  # <<<--- 正式名称

# The 'REGION' variable for file paths is now generated automatically.
REGION = TARGET_OCEAN_REGION.lower().replace(" ", "_").replace("_ocean", "")
# REGION = "equatorial_atlantic"  # <--- 在这里定义区域名称，方便以后改为 "north_atlantic" 等
# =======================
# PATHS (请根据实际情况修改)
# =======================
# 原始数据路径
...
# =====================================================================================
# PART 1: Data Filtering (equatorial_atlantic Region)
# =====================================================================================
def create_equatorial_atlantic_mask(shapefile_path, target_region):
    """
    Loads marine polygons and creates a single geometric mask for the equatorial_atlantic Ocean region.
    """
    print("Step 1: Creating precise equatorial_atlantic geographic mask...")
    try:
    ...
    ocean_mapping = {
        "Atlantic Ocean": [
            ...
            "Davis Strait", "Bahía de Campeche", "Baffin Bay"
        ]
    }
    ...
    # Case 1: The region is one of the Atlantic subdivisions that require splitting.
    if target_region in ["North Atlantic Ocean", "Equatorial Atlantic Ocean", "South Atlantic Ocean"]:
        print("Applying complex Atlantic Ocean splitting rules...")
        atlantic_polygons = marine_gdf[marine_gdf['main_basin'] == 'Atlantic Ocean'].copy()
        if atlantic_polygons.empty:
            print("No Atlantic Ocean polygons found.")
            return None
        ...
        for _, row in atlantic_polygons.iterrows():
            geom, name = row.geometry, row['name']

            # Special Rule: Caribbean Sea excludes its equatorial part
            if name == 'Caribbean Sea':
                if target_region == 'North Atlantic Ocean':
                ...
                elif target_region == 'South Atlantic Ocean':
            ...
            # General Rule for all other Atlantic seas
            if target_region == 'North Atlantic Ocean':
                north_intersection = geom.intersection(north_zone)
                if not north_intersection.is_empty: geometries_for_mask.append(north_intersection)
            elif target_region == 'Equatorial Atlantic Ocean':
...
def get_equatorial_atlantic_data():
    # 1. Load Raw
    ...
    # 3. Geo Filter
    # equatorial_atlantic_mask = create_equatorial_atlantic_mask(SHAPEFILE_PATH)
    equatorial_atlantic_mask = create_equatorial_atlantic_mask(SHAPEFILE_PATH, TARGET_OCEAN_REGION)
    print(f"Filtering {len(data)} points by equatorial_atlantic mask...")
    ...
    mask_gdf = gpd.GeoDataFrame(geometry=[equatorial_atlantic_mask], crs="EPSG:4326")
    ...
    print(f"equatorial_atlantic Data Count: {len(final_data)}")
...
def main():
    t0 = time.time()
    print("===== equatorial_atlantic INTEGRATED PIPELINE STARTED =====")

    # 1. Data
    data = get_equatorial_atlantic_data()
