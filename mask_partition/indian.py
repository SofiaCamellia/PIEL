# -*- coding: utf-8 -*-
"""
indian INTEGRATED PIPELINE:
Geographic Filter -> 6 Base Models -> Spatial Stacker -> PINN
------------------------------------------------------------
1. Filters IAP data for indian Ocean using shapefile.
...
# =====================================================================================
# PART 1: Data Filtering (indian Region)
# =====================================================================================
def create_indian_mask(shapefile_path):
    """
    Loads marine polygons and creates a single geometric mask for the indian Ocean region.
    """
    print("Step 1: Creating precise indian geographic mask...")
    try:
    ...
    indian_gdf = marine_gdf[marine_gdf['main_basin'] == 'indian Ocean'].copy()
    if indian_gdf.empty:
        print("No indian polygons found in the shapefile.")
        return None
    indian_mask = indian_gdf.unary_union
    print("indian geographic mask created successfully!")
    return indian_mask
...
def get_indian_data():
    ...
    # 3. Geo Filter
    # indian_mask = create_indian_mask(SHAPEFILE_PATH)
    indian_mask = create_indian_mask(SHAPEFILE_PATH)
    print(f"Filtering {len(data)} points by indian mask...")
    ...
    mask_gdf = gpd.GeoDataFrame(geometry=[indian_mask], crs="EPSG:4326")
    ...
    print(f"indian Data Count: {len(final_data)}")
...
def main():
    t0 = time.time()
    print("===== indian INTEGRATED PIPELINE STARTED =====")
