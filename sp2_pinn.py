...
MAX_MUL_SHALLOW = 1.5
MAX_MUL_MID = 1.15
NEG_PENALTY_W = 5.0   # 清洗负值后建议降到 0~5；如果你仍想严格非负就保留
...
#     def __init__(self):
#         super(AvgPINN, self).__init__()
#         self.input_size = 1 + 4
#         # [修改] 大幅增加神经元数量，适应全球复杂性
#         self.net = nn.Sequential(
#             nn.Linear(self.input_size, 256), # 32 -> 256
#             nn.BatchNorm1d(256),
#             nn.Tanh(),                       # ReLU 容易死掉，PINN 常用 Tanh
#             nn.Linear(256, 128),
#             nn.BatchNorm1d(128),
#             nn.Tanh(),
#             nn.Linear(128, 64),
...
#             nn.Linear(64, 1)
#         )
#     # 底线（neg_penalty）：确实“失去意义”了，因为 Softplus 保证了 y_pred 恒大于 0，所以 neg_penalty 恒为 0。这一项你可以放心地从 Loss 函数里删掉。
#     # >0这一项可以不学，直接通过softplus来约束。
#     def forward(self, stack_pred, physical_features):
#         x = torch.cat([stack_pred, physical_features], dim=1)
#         return self.net(x).squeeze()
class AvgPINN(nn.Module):
    def __init__(self):
        super(AvgPINN, self).__init__()
        self.input_size = 1 + 4  # Average预测值 + 物理特征
    ...
    def forward(self, avg_pred, physical_features):
        x = torch.cat([avg_pred, physical_features], dim=1)
        x = self.fc1(x)
        x = self.bn1(x)
        x = self.relu1(x)
        x = self.fc2(x)
